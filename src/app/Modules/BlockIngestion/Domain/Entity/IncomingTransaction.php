 ...
 *
 * Что значит «найти поступление» и как это связано с подтверждениями — см.
 * GUIDE.md, Урок 5 «Сканирование цепи» и Урок 6 «Подтверждения и финализация».
 *
 * Количество подтверждений и статус обновляются модулем Confirmation через
 * метод {@see applyConfirmation()}.
 *
...
final class IncomingTransaction
{
     ...
     * Вызывается модулем Confirmation; ничто за пределами этого агрегата не имеет
     * права менять статус.
     *
     * ⚠️ Инвариант: число подтверждений НИКОГДА не уменьшается (GUIDE.md, Урок 6
     * — конец «Кто это запускает»). Откат при реорге — отдельный путь Orphaned
     * через модуль ReorgDetection, а не «уменьшение счётчика».
     */

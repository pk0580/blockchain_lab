...
Stack: **Laravel 13**, **PHP 8.4**. Code lives under `{APP_ROOT}/`.
...
9. **Secrets and config**
   - `env(...)` outside `{APP_ROOT}/config/*.php`.
...
3. Apply the relevant checks. Use Grep for risky patterns:
   ...
   - `whereRaw\(`
   - `DB::statement\(`
   - `env\(` outside `{APP_ROOT}/config/`
   - `Log::(info|warning|error|debug)\(.*request\(`
   - `\{!!`
4. For routes, check `{APP_ROOT}/routes/api.php`, `{APP_ROOT}/routes/web.php`, and

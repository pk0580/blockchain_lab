...
- **BIP-44**: соглашение по семантике пути:
  ...
  ```
  Апостроф (`'` или `h`) помечает **hardened**-шаг (`index += 0x80000000`). Что это такое и зачем он критичен для безопасности — разбираем подробно ниже, в отдельном разделе «Что такое hardened и зачем он нужен».
...
### Семантика пути BIP-44

Путь `m / purpose' / coin_type' / account' / change / address_index` — это не случайный набор чисел, а соглашение: у каждого уровня свой строго определённый смысл. Разберём слева направо.

**`m` — мастер-узел (корень дерева).**
Корень всего HD-дерева: мастер-ключ + мастер chain code, полученные из seed (см. раздел «Что такое chain code» ниже). Все остальные ключи — его прямые или косвенные потомки. Строчная `m` означает приватную сторону, заглавная `M` — публичную.

**`purpose'` — какой стандарт описывает остаток пути.** *(hardened)*
Фиксированное число, которое говорит кошельку, по какому BIP читать путь и, как следствие, **какой формат адреса** ожидать:

| `purpose'` | Стандарт | Формат адреса Bitcoin |
|---|---|---|
| `44'` | BIP-44 | legacy P2PKH, `1...` |
| `49'` | BIP-49 | P2SH-SegWit, `3...` |
| `84'` | BIP-84 | native SegWit (bech32), `bc1q...` |
| `86'` | BIP-86 | Taproot, `bc1p...` |

У нас BTC использует `84'` (нужны дешёвые bech32-адреса), а EVM и Tron — `44'`.

**`coin_type'` — какая сеть (монета).** *(hardened)*
Число из реестра **SLIP-44**. Изолирует деревья ключей разных сетей: один и тот же seed даёт независимые наборы адресов для BTC, ETH, Tron.
...
| `60'` | Ethereum |
| `195'` | Tron |
| `1'` | любой testnet (по SLIP-44) |

Важный нюанс про L2: у Polygon, Arbitrum и прочих формально есть собственные номера SLIP-44, но на практике для **всех** EVM-сетей берут `60'` (Ethereum). Причина — ключи и формат адреса у них идентичны, и обычно хотят *один и тот же* `0x...`-адрес во всех EVM-сетях. Наш проект так и делает: EVM = всегда `60'` (см. [Урок 4](#урок-4--l1-l2-и-семейства-сетей)).

**`account'` — номер аккаунта.** *(hardened)*
Позволяет одному seed держать несколько **независимых** под-кошельков: `0'` — личный, `1'` — рабочий, и т.д. Нумерация с нуля. Именно на этой границе работает «противопожарная перегородка» hardened (см. раздел про hardened ниже): компрометация одного аккаунта не вскрывает соседние и не поднимается к мастеру. У нас всегда `0'` — один горячий аккаунт.

**`change` — внешняя или внутренняя ветка.** *(обычный, не hardened)*
Принимает ровно два значения:
- `0` — **external** (внешняя): адреса, которые вы отдаёте наружу для приёма платежей.
- `1` — **internal** (внутренняя): адреса для **сдачи** (change) — куда Bitcoin-кошелёк возвращает остаток самому себе при трате UTXO (см. [Урок 3](#урок-3--транзакция-utxo-против-аккаунта)). Отсюда и название уровня.

В аккаунт-модели (EVM/Tron) «сдачи» нет, но уровень сохраняют равным `0` ради совместимости со стандартом. У нас всегда используется внешняя ветка — см. `DerivationPathFactory::buildExternal()`.

**`address_index` — порядковый номер адреса.** *(обычный, не hardened)*
`0, 1, 2, …` — конкретный адрес внутри ветки `(account, change)`. Каждый новый адрес для приёма = следующий индекс. У нас его выдаёт `AddressRepository::nextDerivationIndex` под advisory-lock (см. «Создание адреса» ниже), чтобы два параллельных запроса не получили одинаковый `{i}`.

**Итог по проекту.** Три семейства, один шаблон — теперь его можно читать как предложение:

| Семейство | Путь | Как читается |
|---|---|---|
| Bitcoin | `m/84'/0'/0'/0/{i}` | BIP-84 · BTC · аккаунт 0 · внешняя · адрес `i` |
| EVM | `m/44'/60'/0'/0/{i}` | BIP-44 · ETH · аккаунт 0 · внешняя · адрес `i` |
| Tron | `m/44'/195'/0'/0/{i}` | BIP-44 · Tron · аккаунт 0 · внешняя · адрес `i` |

### Что такое chain code

Термин `chain code` уже несколько раз мелькнул выше — пора объяснить его по-человечески, потому что без него не понять ни деривацию, ни hardened.

**Короткое определение.** Chain code — это дополнительные **32 байта** (256 бит), которые путешествуют рядом с каждым ключом в HD-дереве. Это **не** приватный ключ и **не** публичный — это отдельная порция «секретной соли», управляющая тем, *как* из родителя получаются дети.

**Откуда он берётся.** Помните, BIP-39 превращает мнемонику в 64 байта seed-материала? BIP-32 разрезает эти 64 байта пополам:

```
seed (64 байта)  ──HMAC-SHA512──►  I (64 байта)
                                    ├─ левые 32 байта  = мастер-приватный ключ
                                    └─ правые 32 байта = мастер chain code
```

Дальше на **каждом** шаге деривации история повторяется: HMAC выдаёт 64 байта, левая половина идёт в дочерний ключ, правая половина становится **дочерним chain code**. То есть у каждого узла дерева — свой собственный chain code, унаследованный и преобразованный от родителя.

**Зачем он нужен.** Вспомним формулу вывода ребёнка:

```
t = HMAC-SHA512( chain_code , parent_key || index )
```

`chain_code` здесь — это **ключ HMAC**, тот самый секрет, без которого результат вообще не посчитать. Если бы chain code не было и формула зависела только от `(ключ, index)`, то любой, кто знает ваш публичный ключ, мог бы вывести все ваши дочерние адреса — ведь и `index` (0, 1, 2…), и сама формула общеизвестны. Chain code добавляет 256 бит непредсказуемости: не зная его, нельзя воспроизвести ветвление, даже зная ключ и номер ребёнка.

Аналогия: вывод детей — это рецепт. Родительский ключ — главный ингредиент, `index` — «номер блюда по порядку». Но если бы рецепт был просто «ключ + номер», любой повар с вашим ключом приготовил бы те же самые блюда. Chain code — это **секретная приправа**, подмешанная в рецепт: не зная её точного состава, повторить блюда нельзя. У каждого узла дерева — своя приправа, передаваемая детям.

**Extended key = ключ + chain code.** Именно поэтому в BIP-32 оперируют не «голым» ключом, а **расширенным (extended)** ключом — это пара «ключ + chain code»:

- **xprv** = приватный ключ + chain code (полный секрет; из него выводятся и дочерние ключи, и адреса).
- **xpub** = публичный ключ + chain code (в обычном режиме позволяет выводить дочерние *публичные* ключи, то есть адреса, — но не приватные).

Когда ниже, в разделе про hardened, говорится «утечка xpub раскрывает все адреса» — это именно потому, что xpub несёт в себе chain code: с ним деривация возможна, без него — нет. Сам по себе публичный ключ без chain code детей не выдаёт.

> Вывод про чувствительность: chain code — это не приватный ключ, его утечка сама по себе деньги не крадёт. Но в паре с публичным ключом он раскрывает всё дерево адресов (удар по приватности), а в паре «публичный ключ + один дочерний приватный» — становится частью той самой атаки восстановления мастер-ключа, которую разбираем в следующем разделе. Поэтому chain code берегут так же, как ключ.

### Что такое hardened

**У BIP-32 есть два режима вывода дочернего ключа.** Какой именно применяется — зашито в номер шага (индекс):

- **обычный (normal)** — индекс от `0` до `2³¹−1`;
- **усиленный (hardened)** — индекс от `2³¹` до `2³²−1`.

Числа `2³¹ = 2 147 483 648` писать в пути неудобно, поэтому придумали короткую запись: апостроф. `0'` означает «индекс `0 + 2³¹`», `5'` — «индекс `5 + 2³¹`». Запись `'` и `h` равнозначны (`0'` = `0h`). То есть hardened — это не отдельное число, а **верхняя половина** диапазона индексов, помеченная апострофом.

**Суперспособность обычного режима — «публичная деривация».** В обычном режиме формула устроена так, что **дочерний публичный ключ можно вычислить из одного только родительского публичного ключа** (плюс chain code). Приватный ключ для этого не нужен:

```
обычный режим:   child_PUBLIC = f(parent_PUBLIC, chain_code, index)
```

Зачем это полезно: можно отдать на ваш веб-сервер только **xpub** (extended public key — родительский публичный ключ + chain code), и сервер будет сам генерировать сколько угодно новых адресов для приёма платежей, **никогда не держа ни одного секрета**. Украсть с такого сервера нечего — приватных ключей там физически нет. Это режим «watch-only» (только наблюдение).

**Тёмная сторона обычного режима.** Та же формула, к сожалению, работает и в обратную сторону. Если злоумышленник раздобудет одновременно:

1. родительский **публичный** ключ + chain code (тот самый xpub), И
2. всего **один** дочерний **приватный** ключ (например, утёк приватный ключ одного-единственного адреса),

— то он сможет **восстановить родительский приватный ключ**. А родительский приватный ключ открывает **вообще все** дочерние ключи. Итог: один утёкший адрес + публичная информация = взломан весь кошелёк.

Почему так происходит, на пальцах. Обычный дочерний приватный ключ вычисляется как:

```
child_priv = parent_priv + t        (t — слагаемое, выводимое из ПУБЛИЧНЫХ данных)
```

Злоумышленник знает `child_priv` (утёк) и умеет посчитать `t` (для этого нужны лишь публичный ключ + chain code + индекс, всё из xpub). Тогда:

```
parent_priv = child_priv − t
```

— и дерево разворачивается вверх до самого мастер-ключа.

**Что меняет hardened.** Усиленный режим подменяет один ингредиент формулы: вместо родительского **публичного** ключа в неё подставляется родительский **приватный**:

```
обычный:    t = HMAC-SHA512(chain_code, parent_PUBLIC  || index)
hardened:   t = HMAC-SHA512(chain_code, parent_PRIVATE || index)
```

Из этого вытекают два последствия — одно плохое, одно хорошее:

- **Цена:** публичную деривацию для hardened-шага сделать уже нельзя — чтобы вывести усиленного ребёнка, обязателен родительский приватный ключ. Watch-only через такой шаг не построить.
- **Выгода:** обратная атака ломается. Имея xpub родителя и даже утёкший дочерний приватный ключ, посчитать `t` невозможно — для него теперь нужен сам родительский приватный ключ, которого у злоумышленника нет. Восстановить `parent_priv` не получается, подъём вверх по дереву обрезан.

Образно: hardened — это **противопожарная перегородка** в дереве ключей. Утечка *ниже* перегородки не позволяет подняться *выше* неё.

> Уточнение про угрозу. Сам по себе утёкший xpub (только публичная часть) деньги украсть не даёт — он позволяет лишь *видеть* все ваши адреса и поступления (неприятно для приватности, но не кража). Катастрофа наступает именно в паре «xpub + один дочерний приватный ключ». Hardened закрывает ровно этот сценарий.

**Почему BIP-44 ставит апострофы именно на первых трёх уровнях.** Вспомним путь:
...
- **Верхние три уровня — hardened (перегородки).** Компрометация ключей одного `account'` не должна вскрывать ни мастер-сид, ни соседние аккаунты. Поэтому граница аккаунта усилена.
- **Нижние два уровня — обычные (зона удобства).** Их специально оставили без апострофа, чтобы можно было экспортировать xpub *на уровне аккаунта* (`m/84'/0'/0'`) в watch-only сервис, и тот плодил бы конечные адреса (`change` / `address_index`) без единого секрета. А благодаря hardened-границе аккаунта даже утечка этого аккаунтского xpub не поднимет атакующего ни к мастеру, ни к другим аккаунтам.

**В нашем проекте.** Пути `m/84'/0'/0'/0/{i}` (BTC), `m/44'/60'/0'/0/{i}` (EVM), `m/44'/195'/0'/0/{i}` (Tron) — это ровно та же раскладка. Нюанс: у нас приватные ключи вообще не покидают `signing-svc` (см. следующий раздел), поэтому «публичную деривацию ради watch-only» мы напрямую не эксплуатируем — деривацией всегда занимается сам signing-svc, у него есть приватная часть. Но hardened-граница всё равно ценна как защита в глубину: если ключ уровня аккаунта когда-нибудь утечёт, мастер-сид и остальные аккаунты останутся в безопасности.
...
Существуют две принципиально разные модели «что такое транзакция и баланс».
...
Инвариант: `maxReorgDepth >= requiredConfirmations`. Иначе подтверждённая транзакция оказалась бы «уже неотменимой по нашей логике», но при этом её могла бы откатить сеть — нельзя.
...
2. Подгружаем данные транзакции через порт `App\Modules\Ledger\Domain\Contract\ConfirmedTransactionView`.
3. Через порт `App\Modules\Ledger\Domain\Contract\WalletOwnership` ищем, какому кошельку принадлежит адрес-получатель. Если нет — выбрасываем `WalletOwnershipMissingException` (адрес не зарегистрирован у нас — кейс ошибки в данных).
